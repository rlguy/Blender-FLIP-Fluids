...
from . import compositing_tools_ui
from .. import render
...
class FLIPFLUID_PT_HelperPanelMain(bpy.types.Panel):
    ...
    def draw_command_line_tools_render_frame_panel(self, context, box):
        ...
        if body:
            ...
            row.operator("flip_fluid_operators.helper_open_render_output_folder", text="", icon='FILE_FOLDER')
            row = column.row(align=True)
            row.enabled = not cprops.render_passes
    ...
    def draw_command_line_tools_usd_export_panel(self, context, box):
        hprops = context.scene.flip_fluid_helper

        #
        # Command Line USD Export Panel
        #
        box = box.box()
        header, body = box.panel("command_line_tools_usd_export_panel", default_closed=True)

        row = header.row(align=True)
        row.label(text="USD Export:")
        if body:
            column = body.column(align=True)
            row = column.row(align=True)
            row.operator("flip_fluid_operators.flip_fluids_usd_exporter", text="FLIP Fluids USD Export", icon="EXPORT")
            row.operator("flip_fluid_operators.helper_cmd_usd_export_to_clipboard", text="", icon='COPYDOWN')

            row.operator("flip_fluid_operators.helper_open_usd_output_folder", text="", icon='FILE_FOLDER')
            
            row = column.row(align=True)
            row.operator("flip_fluid_operators.flip_fluids_usd_importer", text="FLIP Fluids USD Import", icon="IMPORT")
            row.label(text="", icon='BLANK1')
            row.label(text="", icon='BLANK1')
        else:
            row = row.row(align=True)
            row.operator("flip_fluid_operators.flip_fluids_usd_exporter", text="Launch USD Export", icon="EXPORT")
            row.operator("flip_fluid_operators.helper_cmd_usd_export_to_clipboard", text="", icon='COPYDOWN')
            row.operator("flip_fluid_operators.helper_open_usd_output_folder", text="", icon='FILE_FOLDER')
    ...
    def draw_command_line_tools(self, context):
        ...
        if body:
            ...
            self.draw_command_line_tools_usd_export_panel(context, body)
...
class FLIPFLUID_PT_HelperPanelCompositingTools(bpy.types.Panel):
    ...
    def draw(self, context):
        compositing_tools_ui.draw_compositing_tools_panel(self, context)
