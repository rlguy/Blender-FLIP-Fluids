...
class FlipFluidObjectProperties(bpy.types.PropertyGroup):
    ...
    def _update_object_type(self, context):
        ...
        if self.object_type == 'TYPE_DOMAIN':
            ...
            if bpy.context.scene.flip_fluid.get_num_fluid_objects() > 1:
                self["object_type"] = 0
                errmsg = "Error: The FLIP Fluids Demo is limited to one Fluid object"
            ...
            if bpy.context.scene.flip_fluid.get_num_inflow_objects() > 1:
                self["object_type"] = 0
                errmsg = "Error: The FLIP Fluids Demo is limited to one Inflow object"
                bpy.ops.flip_fluid_operators.display_error(
                        'INVOKE_DEFAULT',
                        error_message=errmsg,
                        popup_width=600
            ...
            if bpy.context.scene.flip_fluid.get_num_outflow_objects() > 1:
                self["object_type"] = 0
                errmsg = "Error: The FLIP Fluids Demo is limited to one Outflow object"
                bpy.ops.flip_fluid_operators.display_error(
                        'INVOKE_DEFAULT',
                        error_message=errmsg,
                        popup_width=600
