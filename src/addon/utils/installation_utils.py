...
import bpy, os, sys, json, datetime, subprocess, platform
...
SUPPORT_LICENSE_TYPE = "Developer"
SUPPORT_LICENSE_ID = "277XK"
...
IS_LINUX_BLENDER_FLATPAK_INSTALLATION = False
...
def update_mixbox_installation_status():
    ...
    mixbox_lut_filepath = os.path.join(mixbox_directory, lut_filename)
...
def update_linux_blender_flatpak_installation_status():
    # Method not currently needed
    pass

    """
    global IS_LINUX_BLENDER_FLATPAK_INSTALLATION

    prefs = vcu.get_addon_preferences()
    if platform.system() != "Linux" or not prefs.enable_linux_flatpak_compatibility:
        IS_LINUX_BLENDER_FLATPAK_INSTALLATION = False
        return

    if prefs.force_linux_flatpak_compatibility:
        # Due to sandboxing, it may not be possible to automatically detect a Blender Flatpak installation
        # The addon preferences contain an option to force Flatpak compatibility
        IS_LINUX_BLENDER_FLATPAK_INSTALLATION = True
        return

    # If the addon is installed to a path that contains "org.blender.Blender", this is a hint that
    # this is a Flatpak installation of Blender.
    # This check may only be correct for default addon installation locations.
    IS_LINUX_BLENDER_FLATPAK_INSTALLATION = "org.blender.Blender" in __file__
    """


def is_linux_blender_flatpak_installation():
    if platform.system() != "Linux":
        return False
    prefs = vcu.get_addon_preferences()
    return prefs.enable_linux_flatpak_compatibility
...
def load_post():
    ...
    update_linux_blender_flatpak_installation_status()
