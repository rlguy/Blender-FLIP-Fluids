...
import bpy, os, math, mathutils, random, datetime
...
class FlipFluidHelperDeleteDomain(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        # Search for selected domain objects
        for obj in context.selected_objects:
            if obj.flip_fluid.is_active and obj.flip_fluid.is_domain():
                return True

        # Search for orphaned domain objects
        orphaned_domain_objects = context.scene.flip_fluid.get_orphaned_domain_objects()

        if orphaned_domain_objects:
            return True
    ...
    def execute(self, context):
        ...
        orphaned_domain_objects = context.scene.flip_fluid.get_orphaned_domain_objects()
        for bl_domain in orphaned_domain_objects:
            self.delete_domain(context, bl_domain.name)
...
def add_geometry_node_modifier(target_object, resource_filepath, resource_name, insert_after_modifier=None):
    ...
    if insert_after_modifier:
        try:
            # May not be possible depending on context
            old_gn_modifier_index = list(target_object.modifiers).index(insert_after_modifier)
            new_gn_modifier_index = len(target_object.modifiers) - 1
            target_object.modifiers.move(new_gn_modifier_index, old_gn_modifier_index + 1)
        except:
            pass
...
class FlipFluidHelperUpdateGeometryNodeModifiers(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for idx, bl_surface in enumerate(surface_cache_objects):
            ...
            new_gn_modifier = add_geometry_node_modifier(bl_surface, resource_filepath, surface_resource, insert_after_modifier=old_gn_modifier)
            if old_gn_modifier and new_gn_modifier:
                self.transfer_gn_modifier_settings(old_gn_modifier, new_gn_modifier)

        for idx, bl_fluid_particle in enumerate(fluid_particle_cache_objects):
            old_gn_modifier = fluid_particle_gn_modifiers[idx]
            new_gn_modifier = add_geometry_node_modifier(bl_fluid_particle, resource_filepath, fluid_particle_resource, insert_after_modifier=old_gn_modifier)
            if old_gn_modifier and new_gn_modifier:
                self.transfer_gn_modifier_settings(old_gn_modifier, new_gn_modifier)
        ...
        for idx, bl_whitewater in enumerate(whitewater_cache_objects):
            ...
            new_gn_modifier = add_geometry_node_modifier(bl_whitewater, resource_filepath, whitewater_resource, insert_after_modifier=old_gn_modifier)
            if old_gn_modifier and new_gn_modifier:
                self.transfer_gn_modifier_settings(old_gn_modifier, new_gn_modifier)
